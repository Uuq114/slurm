...
#define GRES_CONF_ENV_DCMI SLURM_BIT(9)   /* Set ASCEND_RT_VISIBLE_DEVICES */

#define GRES_CONF_SHARED SLURM_BIT(10)      /* Treat this as a shared GRES */
#define GRES_CONF_ONE_SHARING SLURM_BIT(11) /* Only allow use of a shared GRES \
                                             * on one of the sharing GRES */

#define GRES_CONF_ENV_ONEAPI SLURM_BIT(12)    /* Set ZE_AFFINITY_MASK */
#define GRES_CONF_EXPLICIT SLURM_BIT(13)      /* Don't allocate as part of  \
                                                 whole node alloc if wasn't \
                                                 directly requested */
#define GRES_CONF_FROM_STATE SLURM_BIT(14)    /* Flags from state, blow away once \
                                                 node checks in. */
#define GRES_CONF_GLOBAL_INDEX SLURM_BIT(15)  /* devices use global index */
#define GRES_CONF_AUTODETECT SLURM_BIT(16)    /* Conf was made with Autodetect */
#define GRES_CONF_UPDATE_CONFIG SLURM_BIT(17) /* Flag to update gres config */
 ...
 */
extern int gres_g_node_config_load(uint32_t cpu_cnt, char* node_name, list_t* gres_list, void* xcpuinfo_abs_to_mac, void* xcpuinfo_mac_to_abs);
 ...
 */
extern list_t* gres_g_get_devices(list_t* gres_list, bool is_job, uint16_t accel_bind_type, char* tres_bind_str, int local_proc_id, stepd_step_rec_t* step);
 ...
 */
extern int gres_node_state_pack(list_t* gres_list, buf_t* buffer, uint16_t protocol_version);
/*
 ...
 * IN node_name - name of the node for which the gres information applies
 */
extern int gres_node_state_unpack(list_t** gres_list, buf_t* buffer, char* node_name, uint16_t protocol_version);
 ...
 */
extern int gres_node_count(list_t* gres_list, int arr_len, uint32_t* gres_count_ids, uint64_t* gres_count_vals, int val_type);
...
extern int gres_prep_pack_legacy(list_t* gres_list, buf_t* buffer, uint16_t protocol_version);
 ...
 * IN/OUT buffer - location to read state from
 */
extern int gres_prep_unpack_list(list_t** gres_list, buf_t* buffer, uint16_t protocol_version);

extern int gres_prep_unpack_legacy(list_t** gres_list, buf_t* buffer, uint16_t protocol_version);
 ...
 * RET SLURM_SUCCESS or ESLURM_INVALID_GRES
 */
extern int gres_job_revalidate2(uint32_t job_id, list_t* job_gres_list, bitstr_t* node_bitmap);
 ...
 */
extern int gres_job_state_pack(list_t* gres_list, buf_t* buffer, uint32_t job_id, bool details, uint16_t protocol_version);
 ...
 * IN job_id - job's ID
 */
extern int gres_job_state_unpack(list_t** gres_list, buf_t* buffer, uint32_t job_id, uint16_t protocol_version);
 ...
 */
extern uint32_t gres_job_test(list_t* job_gres_list, list_t* node_gres_list, bool use_total_gres, int core_start_bit, int core_end_bit, uint32_t job_id, char* node_name);
 ...
 */
extern int gres_step_state_pack(list_t* gres_list, buf_t* buffer, slurm_step_id_t* step_id, uint16_t protocol_version);
 ...
 * IN step_id - job and step ID for logging
 */
extern int gres_step_state_unpack(list_t** gres_list, buf_t* buffer, slurm_step_id_t* step_id, uint16_t protocol_version);
 ...
 */
extern void gres_step_state_log(list_t* gres_list, uint32_t job_id, uint32_t step_id);
 ...
 * RET - SLURM_SUCCESS or error code
 */
extern int gres_get_step_info(list_t* step_gres_list, char* gres_name, uint32_t node_inx, enum gres_step_data_type data_type, void* data);
 ...
 */
extern uint32_t gres_flags_parse(char* input, bool* no_gpu_env, bool* sharing_mentioned);
...
extern void gres_add_type(char* type, gres_node_state_t* gres_ns, uint64_t tmp_gres_cnt);
